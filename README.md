...
| command | does |
|---|---|
| `mx init [path]` | scaffold/adopt a runtime (`repos/`, `works/`, `files/`, `.mx-root`, `mx.json`, `CLAUDE.md`, `context/INDEX.json`) |
| `mx info [--all] [--porcelain]` | list repos, works, worktrees, and ports (active works only by default; `--all` includes archived) |
| `mx sync` | re-sync runtime with current mx version: re-stamp `CLAUDE.md`, backfill `context/INDEX.json`, the central `hooks/` hub + runtime `bin/` + runtime `files/` store, each repo's `repo.json`, per-work dirs (`wt/`/`scripts/`/`files/`/`tmp/`/`sessions/`) + work `CLAUDE.md`. Same-major, non-destructive. |
