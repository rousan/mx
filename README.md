...
| `mx sync` | re-sync runtime with current mx version: re-stamp `CLAUDE.md`, backfill `context/INDEX.json`, per-work `sessions/` + `.claude/settings.json`, per-repo `hydrate.sh`/`health.sh` if missing. Same-major, non-destructive. |
