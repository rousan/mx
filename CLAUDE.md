...
- **backfills mx-owned structural directories across every work** — `<work>/wt/`, `scripts/`, `files/`, `tmp/`, `hooks/`, and `sessions/` for any work that pre-dates that scaffolding, plus the runtime-wide `bin/` and its shipped utility bins. Future per-work or per-repo additions slot into `ensureWorkScaffolding` in `@mx/core` and propagate the same way.
- backfills per-repo `hydrate.sh` / `health.sh`, the per-work `CLAUDE.md`, per-work `.claude/settings.json`, and the per-work lifecycle hooks (`hooks/{pre,post}-{archive,unarchive}.sh`) (all stamp-if-missing);
...
**`mx migrate`** upgrades an older-version runtime up to the version this CLI supports — the only runtime command allowed on a version mismatch. It validates the full migration chain before mutating (`NO_MIGRATION` on a gap, `CLI_TOO_OLD` if the runtime is newer). The v1 → v2 step upgrades **both** the repo and work layouts: it moves each clone into `repos/<repo>/git/` (with `git worktree repair`), and restructures every work — moving its flat worktrees into `wt/` (`git worktree move`), creating the new `wt/`/`scripts/`/`files/`/`tmp/`/`sessions/` folders, stamping the work `CLAUDE.md`, and rewriting the `.code-workspace` folder paths to `wt/<repo>`.
...
├── bin/                # runtime-wide utility executables for PATH; mx ships some, user adds more (stamp-if-missing)
...
└── works/<feature>/    # one folder per feature
    ...
    ├── hooks/          # per-work lifecycle hooks: {pre,post}-{archive,unarchive}.sh (stamp-if-missing no-ops)
...
- **`mx sync`** — (formerly `mx update`) re-stamp the runtime `CLAUDE.md`; stamp `context/INDEX.json` only if missing; backfill the runtime `bin/` + shipped utility bins, the per-work dirs (`wt/`, `scripts/`, `files/`, `tmp/`, `hooks/`, `sessions/`), per-repo `hydrate.sh`/`health.sh`, the per-work `CLAUDE.md`, per-work `.claude/settings.json`, and the per-work lifecycle hooks (`hooks/{pre,post}-{archive,unarchive}.sh`) (stamp-if-missing). Never modifies user data. Version-gated.
...
- **`mx migrate [--dry-run]`** — upgrade an older runtime to the supported version (the only runtime command allowed on a version mismatch); validates the chain first (`NO_MIGRATION` / `CLI_TOO_OLD`). v1→v2 moves clones into `git/` (`git worktree repair`) **and** restructures every work: flat worktrees into `wt/`, creates `scripts/`/`files/`/`tmp/`, stamps the work `CLAUDE.md`, rewrites `.code-workspace` paths to `wt/<repo>`. `--dry-run` validates and prints the full plan (every path it would change) without mutating anything — porcelain adds `dryRun: true`.
- **`mx help` / `mx version`**.
...
- **Done and verified:** the full TS pnpm monorepo (`@mx/core` source + `@mx/cli` source + `npm/` publishable package), all commands (`init`, `info`, `sync`, `update`, `migrate`, `repo`, `work` incl. `worktree`/`port`/`path`/`open`), runtime versioning + container repo layout (v2), env-based runtime discovery, templates copied into `npm/templates/` at build time, CI workflow for PR checks, `scripts/release.sh` for local publishing, MIT license, and a consumer README at `npm/README.md`. `pnpm typecheck/lint/test/build` are green; the packed tarball installs via `npm i -g` and runs self-contained from outside the repo. Hosted at `github.com/roulabs/mx`, branch `main`.
