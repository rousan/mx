...
- **`mx doctor [--install [--yes]]`** — environment check for the tmux workflow: verifies the **required** deps (tmux, neovim, claude, git) and a **recommended** editor toolbelt (ripgrep, fd, fzf, bat, lazygit, eza, zoxide), prints the exact install command for anything missing (per detected package manager — brew/apt/dnf/pacman), checks (when tmux-resurrect is in use) whether the `mx-tmux-resurrect-filter` is wired as `@resurrect-hook-post-save-all` — ✓ wired / ⚠ with the one-line fix (reads both the live tmux option and `~/.tmux.conf`), and (when a runtime is present) reports the **context-index size** vs Claude Code's ~150k `@import` cap — ⚠ when `context/INDEX.json` is approaching (≥85%) or over the limit (Claude Code may drop the tail), so it's trimmed before entries are silently lost. `--install` runs the install command (confirm, or `--yes`). Core check: `contextIndexStatus` in `packages/core/src/context.ts`. Not version-gated; touches no runtime state. Implemented in `apps/cli/src/commands/doctor.ts`.
- **`mx help` / `mx version`**.
