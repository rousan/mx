...
│   └── {pre,post}-{worktree-create,worktree-remove,work-archive,work-unarchive,repo-fetch} · repo-health · work-health · session-prompt · work-session
...
- **`mx doctor [--install [--yes]]`** — environment check for the tmux workflow: verifies the **required** deps (tmux, neovim, claude, git) and a **recommended** editor toolbelt (ripgrep, fd, fzf, bat, lazygit, eza, zoxide), prints the exact install command for anything missing (per detected package manager — brew/apt/dnf/pacman), and (when a runtime is present) reports the **context-index size** vs Claude Code's ~150k `@import` cap — ⚠ when `context/INDEX.json` is approaching (≥85%) or over the limit (Claude Code may drop the tail), so it's trimmed before entries are silently lost. `--install` runs the install command (confirm, or `--yes`). Core check: `contextIndexStatus` in `packages/core/src/context.ts`. Not version-gated; touches no runtime state. Implemented in `apps/cli/src/commands/doctor.ts`.
- **`mx help` / `mx version`**.
...
**Repos** (clones live at `repos/<repo>/git/`, inside a per-repo container with a `repo.json`) — `mx repo`: `add <git-url> [--name <n>]` (only command that clones; writes `repo.json`) · `new <name> [--quick] [-o] [--description <t>]` (create a fresh **local** repo with no remote: `git init` on `main` + starter README + initial commit + `repo.json`; `--quick` also creates a `dev-<name>` work + a worktree on `develop` and fires `post-worktree-create`, `-o` opens it — a one-shot quick-start for throwaway apps; the worktree forks `main` onto `develop` since the pristine holds `main`) · `ls [--lite]` (shows container path; `--lite` = a compact name + path table, one row per repo; porcelain `path`) · `-n <name> fetch` (fires `pre/post-repo-fetch`; fast-forwards the checked-out **and** base branches; `mx repo fetch --all` fetches every repo) · `-n <name> info` · `health` / `-n <name> health` (same per-repo detail block showing only **metric** rows that carry a ✓/⚠ — current branch (vs default), uncommitted, untracked, ahead/behind (when upstream), last fetched (remote repos, ✓ if within 24h else stale), plus the captured `repo-health` hook output as the `extra` row; the repo name gets an aggregate ✓/⚠; without `-n` it prints every repo's block in turn) · `-n <name> rm` (refuses if any work uses it). Health checks are purely local — they don't fetch; run `mx repo -n <name> fetch` first if you want a fresh comparison against origin.

**Works** — `mx work`: `new <name> [<repo>[:<branch>[:<base>]]]... [--description <t>] [--branch <b>] [--base <ref>] [--agent-managed] [--open|-o]` (creates folder, empty `work.json`, the per-work dirs `wt/`/`scripts/`/`files/`/`tmp/`/`sessions/`, the work `CLAUDE.md`; prints path; `--agent-managed` (alias `--agent-generated`; accepts `=true`/`=false`) stamps `isAgentManaged: true` in `work.json` — an AI agent creating a work for a scoped sub-task passes this so the work is grouped separately and skipped by session tooling; **positional repos after the name create initial worktrees**, each `<repo>[:<branch>[:<base>]]` — per repo: branch = `:<branch>` → `--branch` → the work name, base = `:<base>` → `--base` → pristine HEAD (empty branch like `app::develop` = default branch); repos validated up front so a bad/duplicate repo fails before any work is made; fires `pre/post-worktree-create` per worktree; `-o` builds the work's tmux session and opens it in a new terminal, see `attach`/`open`) · `ls [--all|--archived] [--lite]` (default: active only; shows folder path / porcelain `path`; `--lite` = a compact name + path table, one row per work; **groups into `user` and `agent` sections when agent-created (`isAgentManaged`) works are present** — flat otherwise) · `-n <name> info` · `describe <t>` · `set-agent-managed [true|false]` (flip `isAgentManaged` on an existing work — the fix when an agent forgot `--agent-managed`; value defaults to true, `false` clears it) · `path` · `attach [--prompt <t>]` (the primary way in: **one work == one tmux session** `mx/<name>`; builds it lazily if missing — `main` window with the work's Claude session (left; **resume-or-create keyed to the work name** — one name per work: if a session named `<name>` exists (`findSessionsByName`, covering pre-tmux/hand-run sessions too) run `claude --resume <name>` (Claude resolves the title; picker on the rare duplicate), else create `claude -n <name>`) + `nvim wt` (right; the work's worktrees folder, overridable in the `work-session` hook), and a `run` window with a 2×2 shell grid; seeds `MX_*` env incl. per-port `MX_PORT_*`; fires the `work-session` hook after building — then attaches THIS terminal (`switch-client` inside tmux, else `attach-session`); self-healing after reboot/kill; **interactive-only, so it has no `--porcelain` form** — passing `--porcelain` errors (`INTERACTIVE_ONLY`) and points at `ensure`) · `ensure [--prompt <t>]` (the non-attaching counterpart to `attach`: builds the work's tmux session if missing — same lazy, self-healing build, same `work-session` hook — but **does not attach**, leaving the current terminal alone; here `--porcelain` is a pure output-format flag (never changes behaviour), emitting `{work, session, created, claudeAction, attached:false}`; attach later with `attach`/`open`/`tmux attach -t mx/<name>`) · `open` (alias `-o`: same, but a NEW terminal window — macOS fullscreen Terminal via osascript, Linux via `$MX_TERMINAL`/known emulators; falls back to printing the `attach` line) · `switch [<name>]` (jump between works: with `<name>` it's `attach`; without, an fzf picker over this runtime's live `mx/*` sessions — `attachOrSwitch`) · `gc [--yes]` (prune **orphaned** sessions: live `mx/<work>` sessions whose work — in this runtime, matched by the session's `MX_RUNTIME` — is archived or gone; warns on live panes, confirm or `--yes`; another runtime's sessions are never touched) · `worktree add <repo> [<name>] [--branch <b>] [--base <ref>]` (fires `pre-worktree-create`, then `post-worktree-create`; `<name>` defaults to the repo and is the `wt/<name>` dir + selector — pass a distinct one to hold **multiple worktrees of the same repo**) / `ls` / `rm <worktree>` (fires `pre/post-worktree-remove`) / `set-branch <worktree> [<branch>]` (metadata-only: after you `git checkout` a different branch in the worktree yourself, re-records its **live** branch in `work.json` — mx never checks out; optional `<branch>` guards against a mismatch; `DETACHED`/`BRANCH_MISMATCH`/`NO_WORKTREE` errors) · `port set <worktree> <service> [<port>]` / `unset` / `ls` · `health` / `-n <name> health` (pure-local work-folder health: stray non-mx-native files in the work root, worktree presence vs `work.json`, cross-work port collisions from hand-edits, and archive invariants — an archived work should have its ports freed and worktrees removed; plus the captured `work-health` hook output as `extra`; bare `mx work health` shows every active work, `--all` adds archived) · `archive` (fires `pre/post-work-archive`; removes worktrees **and kills the work's tmux session** — warns first if a pane holds a live foreground process; keeps folder + manifest + sessions + branches; recoverable via `unarchive`; prompts for confirmation — pass `--yes`/`-y` to skip; required for `--porcelain` and non-TTY callers) · `unarchive [<worktree>=<branch>...]` (fires `pre/post-work-unarchive`; re-creates worktrees from `work.json`; positional `<worktree>=<branch>` overrides per-worktree when a recorded branch is missing) · `destroy --force` (PERMANENT: deletes the work folder including session summaries **and kills the tmux session**; branches still kept). `--base` resolves to a commit SHA (trying the ref, then `origin/<ref>`) so a bare branch name forks correctly; `worktree rm` / `archive` / `destroy` refuse on uncommitted changes; ports are unique across **all** works (no blocks). `archive` flips `isArchived: true` and stamps `archived_at` in `work.json`; `unarchive` clears them. All lifecycle hooks are central (`<runtime>/hooks/<event>`); a `pre-*` non-zero exit aborts the op (`HOOK_FAILED`), a `post-*` non-zero exit only warns — see § Hooks.

**Bin** (runtime-wide utility executables at `<runtime>/bin/`, meant for `PATH`) — `mx bin` (alias `mx bins`): `ls` (list bins grouped into **built-in** (mx-shipped) and **user** sections, flag non-executable ones, and note whether `bin/` is on `PATH`; porcelain returns `{dir, onPath, bins[]}` with a `shipped` flag per bin) · `path` (print the `bin/` dir for `export PATH="$(mx bin path):$PATH"`). mx ships `dcs`/`lcs` (delete/list Claude Code sessions by name), `mx-ensure-sessions` (Node bin; build the tmux session for active works **without attaching**, in a chosen order, so they line up in tmux's session picker — via `mx work ensure`; **skips agent-created (`isAgentManaged`) works by default** — `--include-agent` to include them; takes exact names/globs + `-x`/`--except <glob>`, `--dry-run`; with no positional args follows an optional ordered config at `<runtime>/files/mx-ensure-sessions.conf` — work lines, `*` = works not named elsewhere, `exclude:` globs; `--example` prints a starter, `--config`/`$MX_ENSURE_SESSIONS_CONFIG`/`--no-config` control it), and `mx-kill-sessions` (kill every live `mx/*` tmux session at once; confirms unless `-y`) from `templates/bin/`; shipped bins are **re-stamped (overwritten) on every `init`/`sync`** like the runtime `CLAUDE.md` (so updates land), while **user-added bins are never touched**. To customize a shipped bin, copy it to a new name. Distinct from a work's `scripts/` (per-work). Bare `mx bin` defaults to `ls`.
...
- **Semantics.** `pre-*` non-zero exit **aborts** the operation (`HOOK_FAILED`, nothing mutated); `post-*` non-zero is a **warning**; `repo-health` / `work-health` stdout is captured into the `extra` row of `mx repo health` / `mx work health` (cwd = the pristine clone / the work folder). Health hooks follow a **silent-when-healthy** convention: print nothing (or a bare `ok`/`OK`) when fine (renders ✓ `OK`), print the problem when not (renders ⚠ and flags the block). `session-prompt` stdout becomes the initial prompt of a new Claude session created by `mx work attach`/`open` (cwd = the work folder; `MX_WORK` / `MX_WORK_PATH` / `MX_SESSION_NAME`); empty output → clean session, a non-zero exit → output ignored. It fires only on **create**, not on resume, and `--prompt <text>` overrides it. `work-session` fires **after** mx builds a work's tmux session (cwd = the work folder; env adds `MX_TMUX_SESSION` (the tmux target) and `MX_CLAUDE_SESSION_ID`); post-style (non-zero only warns), it lets the user rearrange/extend the default layout (add a window per repo, start a server, swap the editor). Delete it to keep mx's default layout.
...
- **Not done yet:** per-runtime support for non-Claude agents (`AGENTS.md` for Codex, `.cursorrules` for Cursor). Optional next idea: isolated per-env state (separate DB schema / container) for safe parallel runs.
- **Shipped in 4.1.0 — the tmux workflow:** one active work == one tmux session `mx/<work>` (built lazily by `mx work attach`/`open`, a `main` window with the work's Claude session + `nvim` and a `run` window with a 2×2 shell grid), the work's Claude session (resume-or-create keyed to the work NAME — `claude --resume <name>` when a session named `<name>` exists, else `claude -n <name>`; no pinned id), the `work-session` customization hook, session teardown on archive/destroy, `mx work switch` (fzf picker) + `mx work gc` (prune orphaned sessions whose work is archived/gone), and `mx doctor`. macOS + Linux (Windows unsupported). The earlier osascript `-o` terminal/editor layout (2.0.0) was replaced by this. Core naming/id helpers: `packages/core/src/tmux.ts`; CLI orchestration: `apps/cli/src/tmux.ts`.
