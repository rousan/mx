...
- **backfills mx-owned structural directories across every work** — `<work>/wt/`, `scripts/`, `files/`, `tmp/`, `hooks/`, and `sessions/` for any work that pre-dates that scaffolding. Future per-work or per-repo additions slot into `ensureWorkScaffolding` in `@mx/core` and propagate the same way.
...
- **`mx sync`** — (formerly `mx update`) re-stamp the runtime `CLAUDE.md`; stamp `context/INDEX.json` only if missing; backfill the per-work dirs (`wt/`, `scripts/`, `files/`, `tmp/`, `hooks/`, `sessions/`), per-repo `hydrate.sh`/`health.sh`, the per-work `CLAUDE.md`, per-work `.claude/settings.json`, and the per-work lifecycle hooks (`hooks/{pre,post}-{archive,unarchive}.sh`) (stamp-if-missing). Never modifies user data. Version-gated.
