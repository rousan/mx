...
- **`mx update` → `mx sync` (rename).** The old re-stamp command is now `mx sync` (header "Synced runtime at …"); same behavior plus it backfills per-repo `hydrate.sh`/`health.sh` and per-work `.claude/settings.json` (stamp-if-missing). **`mx update` is now a new command** that self-updates the CLI within its major (`npm i -g @roulabs/mx@^<major>`), detects a newer major and suggests the deliberate upgrade, and is *not* version-gated.
