...
- **Runtime newer than the CLI** → upgrade the CLI (`mx update`, then `npm i -g @rousan/mx@<N>` for a new major). `mx migrate` against a newer runtime errors `CLI_TOO_OLD`.
...
- `isAgentManaged` — true when the work was created by an **AI agent** for a scoped sub-task, via `mx work new <name> --agent-managed`. Only present when true (absent for user works). Advisory: `mx work ls` groups agent-created works separately from yours, and `mx-ensure-sessions` skips them (an agent runs its own work's session). Set once at creation.
