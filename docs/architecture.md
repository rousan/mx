...
- `templates.ts` — `stampClaudeMd`, `stampContextIndex`, `stampRepoScripts`, `stampRuntimeBins` (copies `templates/bin/*` into `<runtime>/bin/`, stamp-if-missing + executable, always ensuring the dir exists), `removeStaleRuntimeReadme` (the per-work `CLAUDE.md` is stamped inline by `ensureWorkScaffolding` in `runtime.ts`, not from a template file)
