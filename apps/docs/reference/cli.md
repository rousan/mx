...
| `open` (alias `-o`) | macOS: open a Terminal in the work and resume/create its agent session. |
...
| `archive [--yes]` | Free worktrees + ports; keep branches, folder, and sessions. Recoverable. |
| `unarchive [<worktree>=<branch>…]` | Re-create worktrees from the manifest. |
| `destroy --force` | Permanently delete the work folder (branches still kept). |
