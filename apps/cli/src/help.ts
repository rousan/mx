...
Works (features):
  ...
  mx work -n <name> worktree add <repo> [--branch <b>] [--base <ref>] [--no-setup]   runs the repo's setup.sh after add unless --no-setup
  mx work -n <name> worktree ls [--porcelain]
  mx work -n <name> worktree rm <repo>                  refuses on uncommitted changes; keeps branch
  mx work -n <name> worktree setup <repo>               re-run the repo's setup.sh against its worktree
